%% Load Data
% Measured trajectory
x_m = out.m_CTC.Data(:, 1);
y_m = out.m_CTC.Data(:, 2);    
z_m = out.m_CTC.Data(:, 3); 

% Reference Trayectory
x_ref = reshape(out.ref_CTC.Data(1,:), [], 1);
y_ref = reshape(out.ref_CTC.Data(2,:), [], 1);
z_ref = reshape(out.ref_CTC.Data(3,:), [], 1);
...
n1_CTC = length(time_2s) - 1;
% Ref points
x_ref_2s = x_ref(end-n1_CTC:end);
y_ref_2s = y_ref(end-n1_CTC:end);
z_ref_2s = z_ref(end-n1_CTC:end);
% Mesure points
x_m_2s = x_m(end-n1_CTC:end);
y_m_2s = y_m(end-n1_CTC:end);
z_m_2s = z_m(end-n1_CTC:end);

%% Trajectory graph 
figure;
hold on; 
plot3(x_ref_2s, y_ref_2s, z_ref_2s, 'k', 'LineWidth', 1.5);  
plot3(x_m_2s, y_m_2s, z_m_2s, 'r', 'LineWidth', 1.5);  
hold off;
xlabel('x (m)');
ylabel('Y (m)');
zlabel('Z (m)');
grid on;
axis equal; 
legend('Reference trajectory', 'CTC');
view(3); 
rotate3d on;  
...
hold on;
plot(time_2s, x_m_2s, 'r','LineWidth', 2);   
hold off;
xlim([2 7.5]);
ylabel('x (m)');
grid on;
legend('reference', 'CTC');
% Y Graphics
subplot(3,1,2);
plot(time_2s, y_ref_2s, 'k', 'LineWidth', 2);    
hold on;
plot(time_2s, y_m_2s, 'r','LineWidth', 2);   
hold off;
xlim([2 7.5]); 
ylabel('y (m)');
grid on;
...
hold on;
plot(time_2s, z_m_2s, 'r','LineWidth', 2);    
hold off;
xlim([2 7.5]);
ylabel('z (m)');
grid on;
...
figure;
semilogy(time_2s, eucledian_distance, 'r', 'LineWidth', 2);
hold on;
%hold off;
xlim([2 7.5]);
xlabel('Time (s)');
