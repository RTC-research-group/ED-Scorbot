#include "scorbot_driver_effort/scorbot_joint_driver.h"

namespace scorbot_driver_effort {

const double ScorbotJointDriver::CURRENT_FACTOR = 160.0;

ScorbotJointDriver::ScorbotJointDriver(const std::string &joint_name)
    : EthercatDriver(), _joint_name(joint_name),
  ...
  _set_point.currentLim = (int16_t)std::floor(
      ScorbotJointParametersConfig::__getDefault__().current_limit *
      CURRENT_FACTOR);
  ...
  const std::vector<
      ScorbotJointParametersConfig::AbstractParamDescriptionConstPtr>
      params = ScorbotJointParametersConfig::__getParamDescriptions__();
  for (std::vector<ScorbotJointParametersConfig::
  ...
  // Init RT buffers
  _current_limit_.initRT((int16_t)std::floor(
      ScorbotJointParametersConfig::__getDefault__().current_limit *
      CURRENT_FACTOR));
  _calibration_offset_.initRT((int16_t)std::floor(
      ScorbotJointParametersConfig::__getDefault__().calibration_offset *
      RAD2ENC));
  // Create node handle for dynamic reconfigure
  ros::NodeHandle nh(_name);
  ROS_INFO_STREAM_NAMED(_name, "Initializing dynamic reconfigure in namespace "
                                   << nh.getNamespace());

  // Start dynamic reconfigure server
  _param_reconfig_server.reset(
      new DynamicReconfigServer(_param_reconfig_mutex, nh));
  /* Set dynamic reconfigure callback */
  _param_reconfig_callback =
      boost::bind(&ScorbotJointDriver::dynamicReconfigCallback, this, _1, _2);
  _param_reconfig_server->setCallback(_param_reconfig_callback);
  _dynamic_reconfig_initialized =
      true; /* We avoid first call of Dynamic reconfigure */
  /* Initial values for parameters with dynamic reconfigure */
  uint32_t initial_config_level = 0U;
  scorbot_driver_effort::ScorbotJointParametersConfig initial_config;
  std::map<std::string, uint32_t>::iterator it;
  nh.param<double>(
      "current_limit", initial_config.current_limit,
      ScorbotJointParametersConfig::__getDefault__().current_limit);
  initial_config_level |=
      (it = _level_mapping.find("current_limit")) != _level_mapping.end()
          ? it->second
          : 0;
  dynamicReconfigCallback(initial_config,
                          initial_config_level); /* Call dynamic reconfigure to
                                                    setup current limit */

  /* Get encoder configuration */
  int32_t scorbot_default_encoder = 3 * 160 * 96; /* Default Scorbot value */
  nh.param<int32_t>("encoder_ticks_per_revolution",
                    _encoder_tick_per_revolution, scorbot_default_encoder);
  ROS_INFO_STREAM("Joint " << _joint_name << " has "
                           << _encoder_tick_per_revolution
...
void ScorbotJointDriver::dynamicReconfigCallback(
    scorbot_driver_effort::ScorbotJointParametersConfig &config,
    uint32_t level) {
  ROS_DEBUG_STREAM("Dynamic reconfigure callback recieved with level "
                   << level);
  ...
  if (it != _level_mapping.end() && it->second & level) {
    ...
    ROS_INFO_STREAM_NAMED(_joint_name, "Setting current limit for motor "
                                           << _joint_name << " at "
                                           << config.current_limit << " A ("
                                           << raw_value << " raw value)");
  ...
  if (it != _level_mapping.end() && it->second & level) {
    int16_t raw_value =
        (int16_t)std::floor(config.calibration_offset * RAD2ENC);
    // Update values using RT buffer
    ROS_INFO_STREAM_NAMED(
        _joint_name, "Setting calibration offset for motor "
                         << _joint_name << " at " << config.calibration_offset
...
ScorbotGripperDriver::ScorbotGripperDriver() : EthercatDriver() {
  /* Init struct at zero */
  _set_point.controlRegA = 0U;
  _set_point.controlRegB = 0U;
  _set_point.currentRef = 0;
  _set_point.currentLim =
      (int16_t)std::floor(1.875 * ScorbotGripperDriver::CURRENT_FACTOR);
  _set_point.pidCurrentKp = 0U;
  _set_point.pidCurrentKi = 0U;
  _set_point.pidCurrentKd = 0U;
  _set_point.testFloat = 0.0f;
...
void ScorbotGripperDriver::setCurrent(double target) {
  _set_point.currentRef =
      (int16_t)std::floor(target * ScorbotGripperDriver::CURRENT_FACTOR);
}

double ScorbotGripperDriver::getPosition() {
  return (-_joint_data.encPosition - ScorbotGripperDriver::ENCODER_OFFSET) *
         ScorbotGripperDriver::ENCODER_FACTOR;
}
